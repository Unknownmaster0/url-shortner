...
export async function getClickCountsByUrlIds(
  ...
  return Object.fromEntries(
    rows.map((r) => [r.urlId, { total: r.total, distinct: r.distinct }]),
  );
}
...
export async function getClickAnalyticsForShortCode(
  ...
  const daysSinceCreation = Math.max(
    1,
    Math.ceil(
      (now.getTime() - url.createdAt.getTime()) / (24 * 60 * 60 * 1000),
    ),
  );
  const avgDailyClicks =
    Math.round((stats.totalClicks / daysSinceCreation) * 10) / 10;
